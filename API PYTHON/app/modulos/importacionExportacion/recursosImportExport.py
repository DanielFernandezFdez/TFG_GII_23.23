...
def ActualizarEstadisticasImportacion(libro: Libros):
    estadistica_existente = EstadisticasPorMesAuxiliar.query.filter_by(
        mes=libro.mes_creacion, anyo=libro.anyo_creacion
    ).first()

    if estadistica_existente:
        estadistica_existente.numero_libros = estadistica_existente.numero_libros + 1
        estadistica_existente.numero_visitas_totales = (
            estadistica_existente.numero_visitas_totales + libro.visitas_mensuales
        )
        ...
        for libro in libros:
            cw.writerow(
                [
                    ...
                    libro.puntuacion,
                    libro.ubicacion_estudio,
                    libro.url_imagen,
                    libro.visitas_mensuales,
                    libro.visitas_totales,
                    libro.mes_creacion,
                    libro.anyo_creacion,
                    libro.puntuacion_masculino_generico,
                    libro.puntuacion_menores,
                    libro.puntuacion_adultos,
                    libro.puntuacion_ubicacion,
                    libro.puntuacion_actividades,
                ]
            )
    ...
    def importar_csv(self, archivo):
        ...
        for i, row in enumerate(csv_input):
            ...
            nuevo_libro = Libros(
                ...
                ubicacion_estudio=(
                    row[7] if row[7] != "" & row[7] != "No disponible" else None
                ),
                url_imagen=row[8],
