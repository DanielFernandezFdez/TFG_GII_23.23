...
def obtener_resumen_extendido(isbn):
    ...
    if respuesta.status_code == 200:
        ...
        return "Error al obtener la descripción del libro."



def buscar_libro_amazon(elemento):
 
    headers = ({"User-Agent":'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/121.0.0.0 Safari/537.36','Accept-Languaje':'es; q=0.5'})
    ...
    if respuesta.status_code == 200:
        ...
        primer_resultado = instancia.find('div', {'data-component-type': 's-search-result'})
        ...
        if primer_resultado:
            ...
            if respuesta_libro.status_code == 200:
                ...
                ISBNyeditorial=instancia_libro.find('div', id= 'richProductInformation_feature_div')
                ...
                if ISBNyeditorial:
                    for elem in ISBNyeditorial.find_all('li'):
                        ...
                        div_ISBN10 = elem.find('div', id='rpi-attribute-book_details-isbn10')  
                        div_ISBN13 = elem.find('div', id='rpi-attribute-book_details-isbn13') 
                        div_editorial = elem.find('div', id='rpi-attribute-book_details-publisher') 
                        div_fecha = elem.find('div', id='rpi-attribute-book_details-publication_date') 
                        if div_ISBN10:
                ...
                zonadescrip = instancia_libro.find('div', id='bookDescription_feature_div')
                descrip = zonadescrip.find('div', {'class': 'books-expander-content'})
                if descrip:
            ...
            return informacion
    return "No se encontró el libro o hubo un error en la solicitud"
...
selector=0
isbn_o_titulo = "978-0131103627"
#isbn_o_titulo = "La sociedad de la nieve"
if selector==1:
    if re.match(r'\d{13}',isbn_o_titulo):
        descripcion = buscar_libro_agapea_isbn(isbn_o_titulo)
    else:
        descripcion = buscar_libro_agapea_libro(isbn_o_titulo)

else:
    descripcion = buscar_libro_amazon(isbn_o_titulo)
